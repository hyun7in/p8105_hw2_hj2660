...
In order to combine the demographic and biomarker datasets and retain only the participants who appear in both datasets, we used `inner_join` to merge `MCI_baseline_df` based on the `id` with `mci_amyloid_long`, which we pivoted to long format.
