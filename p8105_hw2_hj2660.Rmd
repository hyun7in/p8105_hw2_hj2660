...
The 'pols_df' has 822 observations and 11 variables and tells about the party affiliation distribution of national politicians from 1947 to 2015. The 'snp_df' has 787 observations and 3 variables, ranging from years 1950 to 2015. The 'unemployment_df' has 816 observations and 3 variables, covering the years from 1948 to 2015. The average unemployment rate for January, starting in the year 1975, during months when the president is a Democrat, is 6.57%. The average unemployment rate for Republicans over the same time period was 6.47%. Missing values in the 'close' and 'unemployment' variables have been replaced with 'NA' values.
...
```{r}
MrTrashWheel_df = 
  readxl::read_excel("./Trash_Wheel/Trash_Wheel_Data.xlsx", sheet = "Mr. Trash Wheel", range = "A2:N549") |>
  janitor::clean_names() |>
...
```{r}
ProfTrashWheel_df =
  readxl::read_excel("./Trash_Wheel/Trash_Wheel_Data.xlsx", sheet = "Professor Trash Wheel", range = "A2:M96") |>
  janitor::clean_names() |>
  mutate(homes_powered = (weight_tons * 500)/30) |>
...
```{r}
GwynTrashWheel_df =
  readxl::read_excel("./Trash_Wheel/Trash_Wheel_Data.xlsx", sheet = "Gwynnda Trash Wheel", range = "A2:K108") |>
  janitor::clean_names() |>
  mutate(homes_powered = (weight_tons * 500)/30) |>
...
```{r, echo=FALSE, results='hide'}
ncol(MrTrashWheel_df)
nrow(MrTrashWheel_df)
ncol(ProfTrashWheel_df)
nrow(ProfTrashWheel_df)
ncol(GwynTrashWheel_df)
nrow(GwynTrashWheel_df)
```
