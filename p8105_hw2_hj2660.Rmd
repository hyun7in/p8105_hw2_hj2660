...
In order to combine the demographic and biomarker datasets and retain only the participants who appear in both datasets, we need to merge `dev_MCI`, which already filtered out empty values of `age_at_onset`, with `mci_amyloid_long`, which we pivoted to long format. To combine these two datasets, we used an `inner_join` based on the `id` column. 
